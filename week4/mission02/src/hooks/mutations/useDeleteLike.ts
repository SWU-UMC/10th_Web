...
function useDeleteLike() {
    return useMutation({
        ...
        onMutate:async (lp) => {
            ...
            // 게시글 데이터를 복사해서 NewLpPost라는 새로운 객체를 만들음
            // 복사하는 가장 큰 이유는 나중에 오류가 발생했을 때 이전 상태로 되돌리기 위해서
            const NewLpPost = {...previousLpPost};

            // 게시글에 저장된 좋아요 목록에서 현재 내가 눌러던 좋아요의 위치를 찾아야함
            const my = queryClient.getQueryData<ResponseMyInfoDto>([
                QUERY_KEY.myInfo
            ]);
            const userId = Number(my?.data.id);
            ...
            // 이렇게하면 UI가 바로 업데이트 되고 사용자가 변화를 확인할 수 있다
