...
const LoginPage = () => {
  ...
  // 7주차 핵심: 로그인 로직을 useMutation으로 전환
  const loginMutation = useMutation({
    ...
    onSuccess: (response) => {
      if (response.data.status) {
        ...
        alert(`${name}님 환영합니다!`);
        navigate('/'); // 성공 시 홈으로 리다이렉션
      }
    },
    onError: (error: any) => {
      alert(error.response?.data?.message || '로그인에 실패했습니다. 다시 확인해주세요! ㅠㅠ');
    }
  });
      ...
      <div className="w-full max-w-sm p-10 bg-white/5 backdrop-blur-xl border border-white/20 rounded-[30px] shadow-2xl relative z-10">
        ...
        <form onSubmit={handleSubmit((data) => loginMutation.mutate(data))} className="flex flex-col gap-4">
          ...
          <button 
            ...
            className="w-full p-4 mt-2 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-500 transition-all shadow-lg shadow-blue-500/30 disabled:bg-gray-600"
          >
